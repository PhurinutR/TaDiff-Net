...
class MetricsCalculator:
    ...
    def calculate_metrics(self, pred_img, gt_img, pred_mask, gt_mask, pred_mask_rf=None, gt_mask_rf=None):
        ...
        for threshold in self.dice_thresholds:
            dice_key = f'dice_{int(threshold*100)}'
            suffix = dice_key.split("_")[1]
            metrics[dice_key] = self._dice_score(pred_mask, gt_mask, threshold=threshold).cpu().detach().numpy()
            metrics[f'ravd_{suffix}'], vol = self.ravd(pred_mask, gt_mask, threshold=threshold)
            metrics[f'vol_{suffix}'] = vol
        
        # Reference mask metrics if provided
        if pred_mask_rf is not None and gt_mask_rf is not None:
            metrics['dice_rf'] = self._dice_score(pred_mask_rf, gt_mask_rf, threshold=0.5).cpu().detach().numpy()
    ...
    @staticmethod
    def _dice_score(pred, truth, threshold=0.5, eps=1e-8):
        """Compute Dice score for binary masks using a probability threshold."""
        pred = (pred >= threshold).float()
        truth = (truth >= threshold).float()
