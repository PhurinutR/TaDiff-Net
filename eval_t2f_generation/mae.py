"""
Evaluate TaDiff FLAIR generation MAE on MU-Glioma-Post test patients.
...
- Stack 3 FLAIR slices and compute MAE
"""
...
def _mae_from_numpy_chw(pred_01: np.ndarray, gt_01: np.ndarray) -> float:
    """
    Compute MAE on stacked 3-slice FLAIR inputs [3,H,W] in [0,1].
    """
    metric = MeanAbsoluteError()
    preds = torch.from_numpy(pred_01).float()
    target = torch.from_numpy(gt_01).float()
...
def run_eval(
    ...
    for patient_id in patient_ids:
        ...
        # Compute one MAE on the stacked 3-channel input.
        mae_val = _mae_from_numpy_chw(pred_chw_01, gt_chw_01)
        ...
        rows.append(
            {
                ...
                "mae_flair_3slice": float(mae_val),
            }
        )
        print(
    ...
    mean_mae = float(np.mean([r["mae_flair_3slice"] for r in rows])) if rows else float("nan")
    std_error = float(np.std([r["mae_flair_3slice"] for r in rows]) / np.sqrt(len(rows)))
    out = {
        "num_patients": len(rows),
        "num_inference_runs": total_runs,
        "diffusion_steps": int(diffusion_steps),
        "num_samples_per_slice": int(num_samples),
        "mean_mae_flair_3slice": mean_mae,
        "patients": rows,
        "std_error": std_error,
    }
    ...
    print(f"Patients: {len(rows)} | Inference runs: {total_runs} | Mean MAE: {mean_mae:.6f}")
    print(f"Standard error: {std_error:.6f}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate TaDiff 3-slice FLAIR MAE on MU-Glioma-Post.")
    ...
    parser.add_argument("--output_json", type=str, default="./eval_t2f_generation/mae_results.json")
