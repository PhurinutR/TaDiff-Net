"""
Evaluate TaDiff FLAIR generation DSC on MU-Glioma-Post test patients.
...
- Stack 3 FLAIR slices as channels and compute per-slice DSC on tumor masks
...
4) Compute per-slice DSC: 2 * |A ∩ B| / (|A| + |B|).
5) Report per-slice DSC, patient average, and dataset average.
"""
...
def _compute_dsc(pred_mask: np.ndarray, gt_mask: np.ndarray, eps: float = 1e-8) -> float:
    pred_bool = pred_mask.astype(bool)
    gt_bool = gt_mask.astype(bool)
    vp = float(np.count_nonzero(pred_bool))
    vg = float(np.count_nonzero(gt_bool))
    denom = vp + vg
    if denom <= eps:
        return 1.0
    inter = float(np.count_nonzero(pred_bool & gt_bool))
...
def _save_patient_visualization(
    ...
    fig.suptitle(f"{patient_id} | DSC intensity + mask comparison", fontsize=12)
    save_path = output_dir / f"{patient_id}_dsc_compare.png"
...
def run_eval(
    ...
    rows = []
    total_runs = 0
    vis_dir = output_json.parent / "dsc_visualizations"
    ...
    for patient_id in patient_ids:
        ...
        for c in range(pred_chw_01.shape[0]):
            ...
            dsc_values.append(_compute_dsc(pred_mask, gt_mask))

        dsc_arr = np.asarray(dsc_values, dtype=np.float64)
        patient_avg = float(np.mean(dsc_arr))
        rows.append(
            {
                ...
                "dsc_flair_3slice_per_slice": [float(v) for v in dsc_arr.tolist()],
                "dsc_flair_3slice_patient_avg": patient_avg,
            }
        )
        ...
        dsc_text = ", ".join(f"{v:.6f}" for v in dsc_arr.tolist())
        print(
            f"{patient_id}: sessions={image_t3d.shape[0]} "
            f"src={src_idx} tgt={target_idx} slices={slices} "
            f"DSC per-slice=[{dsc_text}] patient_avg={patient_avg:.6f}"
        )

    mean_dsc = (
        float(np.mean([r["dsc_flair_3slice_patient_avg"] for r in rows])) if rows else float("nan")
    )
    std_error = float(np.std([r["dsc_flair_3slice_patient_avg"] for r in rows]) / np.sqrt(len(rows)))
    out = {
        ...
        "mean_dsc_flair_3slice_patient_avg": mean_dsc,
        "patients": rows,
        "std_error": std_error,
    }
    ...
    print(f"Patients: {len(rows)} | Inference runs: {total_runs} | Mean DSC: {mean_dsc:.6f}")
    print(f"Standard error: {std_error:.6f}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate TaDiff 3-slice FLAIR DSC on MU-Glioma-Post.")
    ...
    parser.add_argument("--output_json", type=str, default="./eval_t2f_generation/dsc_results.json")
    parser.add_argument("--diffusion_steps", type=int, default=600)
    parser.add_argument("--num_samples", type=int, default=1)
    parser.add_argument("--device", type=str, default="cuda:0" if torch.cuda.is_available() else "cpu")
    parser.add_argument("--seg_backend", type=str, default=os.getenv("DSC_SEG_BACKEND", "flair_adaptive"))
    parser.add_argument(
        "--seg_threshold",
        type=float,
        default=float(os.getenv("DSC_SEG_THRESHOLD", "0.5")),
        help="Used by threshold/torchhub_lgg_unet backends.",
    )
    parser.add_argument(
        "--print_mask_stats",
        action="store_true",
        default=os.getenv("DSC_PRINT_MASK_STATS", "1") != "0",
    )
    parser.add_argument(
        "--save_visualization",
        action="store_true",
        default=os.getenv("DSC_SAVE_VIS", "1") != "0",
    )
    parser.add_argument(
